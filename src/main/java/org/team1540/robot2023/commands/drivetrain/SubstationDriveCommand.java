...
class SubstationDriveCommand extends SequentialCommandGroup {
    public SubstationDriveCommand(Drivetrain drivetrain, double yOffset) {
        ...
        Translation2d endPoint = aprilTagLayout.getTagPose(5).orElseThrow().toPose2d().getTranslation().plus(new Translation2d(-Constants.Auto.hpOffsetX, yOffset));
...
//                new PathPoint(layout.getTagPose(tag).orElse(new Pose3d(drivetrain.getPose())).toPose2d().getTranslation(), Rotation2d.fromDegrees(0), Rotation2d.fromDegrees(0)) // position, heading(direction of travel), holonomic rotation
                new PathPoint(endPoint.plus(new Translation2d(-0.127, 0)), Rotation2d.fromDegrees(0), Rotation2d.fromDegrees(0)), // position, heading(direction of travel), holonomic rotation
        ...
        addCommands(drivetrain.getPathCommand(trajectory), new PrintCommand("Done moving"));
