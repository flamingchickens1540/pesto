...
public class SwerveModule {
    ...
    final SimpleMotorFeedforward feedforward = new SimpleMotorFeedforward(Constants.Swerve.driveKS, Constants.Swerve.driveKV, Constants.Swerve.driveKA);
    ...
    private void setAngle(SwerveModuleState desiredState, boolean forceTurn){
        Rotation2d angle = ((Math.abs(desiredState.speedMetersPerSecond) <= (Constants.Swerve.maxVelocity * 0.01)) && !forceTurn) ? lastAngle : desiredState.angle; //Prevent rotating module if speed is less than 1%. Prevents Jittering.
