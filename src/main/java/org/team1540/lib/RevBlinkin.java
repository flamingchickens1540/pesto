...
public class RevBlinkin {
     ...
     *                                   exist.
     */
    public RevBlinkin(int channel, ColorPattern initialPattern) {
        pwm = new Spark(channel);
        this.setPattern(initialPattern);
    }

    public RevBlinkin(int channel) {
        this(channel, ColorPattern.WAVES_PARTY);
    }

    /**
     * Sets the Blinkin's light pattern.
     *
    ...
    /**
     * Creates a new instant command to set the Blinkin's pattern
     * @param pattern the pattern to use
     * @return the command
     */
    public Command commandSetPattern(ColorPattern pattern) {
        return new InstantCommand(() -> this.setPattern(pattern)).ignoringDisable(true);
    }
